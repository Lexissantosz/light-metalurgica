(() => {
  ...
  function normalizarItem(item) {
    return { id: Number(item.id), nome: String(item.nome || "Equipamento").trim() };
  }
  ...
  function removerItem(id) {
    salvarLista(lerLista().filter((item) => Number(item.id) !== Number(id)));
  }
  ...
  function obterIdDoLink(link) { return link ? obterIdDaUrl(link.href) : null; }

  function extrairItem(container) {
    const link = container.querySelector('a[href*="produto.html?id="]');
    let id = obterIdDoLink(link);
    if (!id && container.matches(".product-detail-info")) id = obterIdDaUrl(window.location.href);
    if (!id) return null;
  ...
  function adicionarBotoesInline() {
    document.querySelectorAll(".catalog-card, .product-detail-info, .digital-catalog-card").forEach(criarBotaoInline);
    atualizarBotoesInline();
  }
  ...
  function atualizarBotoesInline() {
    ...
    document.querySelectorAll(".interest-inline-button").forEach((botao) => {
      const adicionado = ids.has(Number(botao.dataset.interestId));
  ...
  function criarWidget() {
    ...
    widget.innerHTML = `
      <button type="button" class="interest-toggle" id="interestToggle" aria-expanded="false" aria-controls="interestDrawer">
        Lista de interesse <span class="interest-count" id="interestCount" aria-live="polite">0</span>
      </button>
      <p class="interest-feedback" id="interestFeedback" role="status" aria-live="polite" hidden></p>
      <section class="interest-drawer" id="interestDrawer" aria-label="Lista de interesse para orçamento" hidden>
        ...
        <div class="interest-drawer-actions">
          <a href="contato.html?lista=1" class="btn btn-gold" id="interestRequest">Solicitar orçamento da lista</a>
          <a href="#" class="btn btn-outline" id="interestWhatsApp" hidden>Enviar lista pelo WhatsApp</a>
          <button type="button" class="interest-clear" id="interestClear">Limpar lista</button>
        </div>
      </section>`;
    document.body.appendChild(widget);

    document.getElementById("interestToggle")?.addEventListener("click", alternarDrawer);
    document.getElementById("interestClose")?.addEventListener("click", fecharDrawer);
    document.getElementById("interestClear")?.addEventListener("click", limparLista);
    document.getElementById("interestDrawer")?.addEventListener("click", (event) => {
      const remover = event.target.closest("[data-interest-remove]");
      if (remover) removerItem(Number(remover.dataset.interestRemove));
    });
    document.addEventListener("keydown", (event) => { if (event.key === "Escape") fecharDrawer(); });
    renderizarWidget();
  }
  ...
  function alternarDrawer() {
    ...
    if (abrir) document.getElementById("interestClose")?.focus();
  }
  ...
  function configurarWhatsApp(lista) {
    ...
    if (!numero || lista.length === 0) { link.hidden = true; return; }
    const somenteDigitos = String(numero).replace(/\D/g, "");
    if (!somenteDigitos) { link.hidden = true; return; }
      ...
      items.innerHTML = lista.length
        ? lista.map((item) => `
            <article class="interest-item">
              <div><strong>${escaparHtml(item.nome)}</strong><a href="produto.html?id=${item.id}">Ver equipamento</a></div>
              <button type="button" class="interest-remove" data-interest-remove="${item.id}" aria-label="Remover ${escaparHtml(item.nome)} da lista">Remover</button>
            </article>`).join("")
        : `<p class="interest-empty">Sua lista está vazia. Adicione equipamentos do catálogo para solicitar vários modelos de uma vez.</p>`;
    }
    if (request) {
  ...
  function preencherContatoDaLista() {
    ...
    if (assunto && !assunto.value.trim()) assunto.value = "Orçamento - lista de interesse";
    if (mensagem && !mensagem.value.trim()) mensagem.value = criarMensagemLista(lista);
  }
  ...
  function observarConteudoDinamico() {
    ...
    const observer = new MutationObserver(() => {
      if (agendado) return;
      agendado = true;
      requestAnimationFrame(() => { adicionarBotoesInline(); agendado = false; });
    });
  ...
  document.addEventListener("light:contact-ready", renderizarWidget);
})();
