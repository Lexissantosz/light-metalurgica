(() => {
  ...
  function urlWhatsApp(numero, mensagem = "Olá! Gostaria de solicitar um orçamento.") {
    ...
    return digitos ? `https://wa.me/${digitos}?text=${encodeURIComponent(mensagem)}` : null;
  }
  ...
  function renderizarVendedores(vendedores) {
    ...
    container.innerHTML = validos.map((vendedor) => {
      ...
      return `
        <article class="contact-card contact-seller-card">
          ...
          <a href="${link}" target="_blank" rel="noopener noreferrer">Chamar no WhatsApp</a>
        </article>`;
    }).join("");
  ...
  function aplicar(config) {
    ...
    configurarLinks("[data-contact-email-link]", config.email ? `mailto:${config.email}` : null);

    const temVendedores = renderizarVendedores(config.vendedores);
    const temCanalDireto = Boolean(whatsappUrl || instagramUrl || config.email || temVendedores);
